        ...
        if final_direction == "UP":
            if days_to_rev <= 3:
                ...
                order_window = (
                    ...
                    f"prices trending UP before {next_rev.strftime('%d-%m-%Y')} revision"
                )
        elif final_direction == "DOWN":
            order_window = (
                f"Wait for revision on {next_rev.strftime('%d-%m-%Y')} -- "
                f"prices may drop (save {days_to_rev} days)"
            )
        else:
            order_window = (
                f"Normal timing -- next revision {next_rev.strftime('%d-%m-%Y')} "
                f"({days_to_rev} days away). No urgency."
            )

        return {
            "next_revision_date": next_rev.strftime("%d-%m-%Y"),
