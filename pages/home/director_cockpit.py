...
def _steps(cur):
    st_list = [("1","Market Snapshot"),("2","Today's Targets"),("3","Manual Price Entry"),("4","Price Calculator"),("5","Send Quote")]
    h = '<div class="ck-st">'
    for n, lb in st_list:
        i = int(n)
...
def _step2():
        ...
        for i, ld in enumerate(leads[:8]):
            ...
            with ca:
                if st.button("Select \u2192", key=f"l{i}"):
                    st.session_state["_ck_c"] = cl; st.session_state["_ck"] = 4; st.rerun()
    ...
    with b2:
        if st.button("Next \u2192 Manual Entry", type="primary", key="s2n"): st.session_state["_ck"] = 3; st.rerun()


# ─── Step 3 ─────────────────────────────────────────────────────────────────

def _step3():
    st.markdown('<div class="ck-sec"><div class="ic" style="background:#FCE7F3;">&#9997;</div><div><div class="tt">Manual Price Entry</div><div class="su">Override pricing and log quotes directly</div></div></div>', unsafe_allow_html=True)
    st.markdown("")
    
    from command_intel.manual_entry import render as render_manual_entry
    render_manual_entry()
    
    st.markdown("")
    b1, b2 = st.columns(2)
    with b1:
        if st.button("\u2190 Back", key="s3bb_back"): st.session_state["_ck"] = 2; st.rerun()
    with b2:
        if st.button("Next \u2192 Calculator", type="primary", key="s3nn_next"): st.session_state["_ck"] = 4; st.rerun()
...
def _step5():
    ...
    if not q:
        st.warning("No quote. Go back to Step 4.")
        if st.button("\u2190 Back", key="s4b0"): st.session_state["_ck"] = 4; st.rerun()
        return
    ...
    b1, b2 = st.columns(2)
    with b1:
        if st.button("\u2190 Back", key="s4b"): st.session_state["_ck"] = 4; st.rerun()
    with b2:
