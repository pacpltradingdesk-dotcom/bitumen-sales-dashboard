...
""", unsafe_allow_html=True)
    ...
    with qa4:
        if st.button("📤 Rate Broadcast", use_container_width=True, key="cc_broadcast"):
            _go("📡 Rate Broadcast")
    with qa5:
        if st.button("📝 Manual Price", use_container_width=True, key="cc_manual"):
    ...
    # Filter: Only bitumen/oil/infrastructure related news
    import re as _re
    _NEWS_KW = _re.compile(
        r'(bitumen|crude|oil|brent|wti|opec|petroleum|refiner|fuel|diesel|petrol|'
        r'nhai|highway|road|infra|construction|cement|tar|asphalt|'
        r'pipeline|tanker|freight|shipping|port|import|export|trade|'
        r'iran|saudi|gulf|middle.east|sanction|energy|lng|gas|'
        r'morth|pmgsy|nhdp|bharatmala|gati.shakti|pwd|'
        r'rupee|dollar|forex|currency|budget|gdp|rbi|'
        r'iocl|bpcl|hpcl|cpcl|mrpl|ongc|reliance.ind|vedanta|cairn)',
        _re.I
    )
    if news_data:
        _filtered = []
        for n in news_data:
            if not isinstance(n, dict):
                continue
            text = (n.get("title", "") or n.get("headline", "")) + " " + (n.get("summary", "") or "")
            if _NEWS_KW.search(text):
                _filtered.append(n)
            if len(_filtered) >= 20:
                break
        news_to_show = _filtered
    else:
        news_to_show = []
  ...
  setTimeout(function(){
    ...
    var html='<div id="pps-news-modal" style="position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:999999;display:flex;align-items:flex-start;justify-content:center;padding-top:60px;">'
      ...
      +'<div style="position:relative;background:#fff;border-radius:20px;width:95%;max-width:720px;max-height:82vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 25px 80px rgba(0,0,0,0.35);animation:ppsZoomIn 0.3s cubic-bezier(0.34,1.56,0.64,1);">'
        ...
        +(a.img?'<div style="width:100%;border-radius:20px 20px 0 0;background:#0F172A;text-align:center;line-height:0;"><img src="'+a.img+'" style="max-width:100%;max-height:350px;display:inline-block;border-radius:20px 20px 0 0;" onerror="this.parentElement.style.height=\\x27160px\\x27;this.parentElement.style.lineHeight=\\x27160px\\x27;this.outerHTML=\\x27<div style=text-align:center;color:#fff><div style=font-size:2.5rem;margin-bottom:8px>\\ud83d\\udcf0</div><div style=font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;opacity:0.8>'+a.s+'</div></div>\\x27"/></div>':'<div style="width:100%;height:160px;background:'+grad+';border-radius:20px 20px 0 0;display:flex;align-items:center;justify-content:center;"><div style="text-align:center;color:#fff;"><div style="font-size:2.5rem;margin-bottom:8px;">\\ud83d\\udcf0</div><div style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;opacity:0.8;">'+a.s+'</div></div></div>')
