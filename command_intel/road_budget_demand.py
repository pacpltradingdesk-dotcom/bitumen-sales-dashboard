...
try:
    ...
    def format_inr(v, sym=True): return f"₹ {float(v):,.2f}"
...
def render():
    ...
    with tabs[0]:
        ...
        top5["Total_Budget_Cr"] = top5["Total_Budget_Cr"].apply(lambda x: f"₹ {x:,} Cr")
    ...
    with tabs[1]:
        ...
        for col in ["MoRTH_Total_Cr", "NHAI_Cr", "PMGSY_Cr"]:
            budget_disp[col] = budget_disp[col].apply(lambda x: f"₹ {x:,} Cr")
        ...
        b4.metric("Expenditure to Date", f"₹ {bm['expenditure_to_date_cr']:,} Cr", "As of Mar-2024")
        st.warning(f"Original budget: ₹ {bm['original_budget_cr']:,} Cr → Revised: ₹ {bm['revised_budget_cr']:,} Cr — cost overrun of ~105%. Revised completion: {bm['revised_completion']}.")
    ...
    with tabs[2]:
        ...
        st.caption(f"Model: Budget × {BITUMEN_PCT_OF_COST*100:.0f}% cost-share ÷ ₹{AVG_BITUMEN_PRICE_PER_MT:,}/MT. Sources: PRS India, BilTrax, MoRTH")

        disp = STATE_DATA.copy()
        disp["Central_Cr"] = disp["Central_Cr"].apply(lambda x: f"₹ {x:,} Cr")
        disp["State_Cr"] = disp["State_Cr"].apply(lambda x: f"₹ {x:,} Cr")
        disp["Total_Budget_Cr"] = disp["Total_Budget_Cr"].apply(lambda x: f"₹ {x:,} Cr")
    ...
    with tabs[4]:
        ...
        dept_disp["Budget_Cr"] = dept_disp["Budget_Cr"].apply(lambda x: f"₹ {x:,} Cr")
...
- Budget increase + crude spike cancel out. Net effect on your volume: minimal. Focus on margin protection.
    ...
    with tabs[7]:
        ...
        assumptions = [
            ...
            ("Average bitumen price (all grades)", f"₹ {AVG_BITUMEN_PRICE_PER_MT:,}/MT", "Blended average of VG30 (₹42,000-44,000), VG40 (₹44,000-46,000), Import (₹38,000-42,000). Dynamic — update in Data Manager."),
...
High logistics complexity. Only for experienced operators with NE network.
        """)
        st.info(f"**Last Updated:** {datetime.date.today().strftime('%d-%m-%Y')} | All financial data in ₹ Crore | Volume in Metric Tonnes (MT)")
