...
def _now_ist() -> str:
    try:
        ...
        return datetime.datetime.now().strftime("%d-%m-%Y %H:%M IST")
...
def _render_future_view(df: pd.DataFrame):
    ...
    try:
        from api_manager import get_brent_price, get_usdinr
        brent = get_brent_price() or 72.87
        usdinr = get_usdinr() or 86.9
    except Exception:
        brent, usdinr = 72.87, 86.9

    brent_impact  = round((brent - 70) * 120 * 0.25, 0)
    fx_impact     = round((usdinr - 84) * 80 * 0.25, 0)
    fo_impact     = round(-80.0, 0)
    freight_impact = 0
    ...
    st.caption(
        f"📌 PASS band: ±₹ {PASS_RUPEE_BAND:,} or ±{PASS_PCT_BAND}% · "
...
def _render_past_performance(df_past: pd.DataFrame):
    ...
    k4.metric("Avg Error (₹)",    f"₹ {avg_err:,.0f}")
    k5.metric("Max Error (₹)",    f"₹ {max_err:,.0f}")

    # PASS/FAIL band note
    st.info(
        f"**PASS Criteria:** Error within **±₹ {PASS_RUPEE_BAND:,}** OR **±{PASS_PCT_BAND}%**  "
    ...
    st.caption(
        f"📌 PASS = |Error| ≤ ₹{PASS_RUPEE_BAND:,} OR |Error%| ≤ {PASS_PCT_BAND}% · "
...
""", unsafe_allow_html=True)
    ...
    if not next_revs.empty:
        ...
        s2.metric("Predicted (₹/MT)", format_inr(nr["Predicted (₹/MT)"]))
        s3.metric("Low Estimate",     format_inr(nr["Low Range"]))
        s4.metric("High Estimate",    format_inr(nr["High Range"]))
        s5.metric("Confidence",       f"{nr['Confidence %']}%")
        st.markdown("---")
