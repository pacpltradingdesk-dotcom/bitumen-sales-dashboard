...
def _ts() -> str:
    return _now_ist().strftime("%d-%m-%Y %H:%M IST")


def _expiry(hours: int) -> str:
    return (_now_ist() + datetime.timedelta(hours=hours)).strftime("%d-%m-%Y %H:%M IST")
...
class RecommendationEngine:
    ...
    def get_latest_recommendations(self) -> dict:
        ...
        if history and isinstance(history, list):
            ...
            if expires:
                try:
                    exp_dt = datetime.datetime.strptime(expires, "%d-%m-%Y %H:%M IST")
