...
try:
    ...
    def format_inr_short(v):
        ...
        return f"₹ {v:,.0f}"
    def format_date(d): return d.strftime("%d-%m-%Y") if hasattr(d, "strftime") else str(d)
...
def _append_changelog(entity, field, old_val, new_val, reason, source_url, project_id="", actor="System"):
    cl = get_changelog()
    cl.append({
        "changed_on_ist": datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S IST"),
...
def _tab_projects():
    ...
    with st.expander("📝 Add Project (Manual Entry)", expanded=False):
        ...
        with st.form("add_project_form"):
            ...
            f9, f10      = st.columns(2)
            start_dt     = f9.text_input("Start Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
            end_dt       = f10.text_input("End Date (DD-MM-YYYY)", (datetime.date.today()+datetime.timedelta(days=730)).strftime("%d-%m-%Y"))
            ...
            source_date  = st.text_input("Source Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
        ...
        if submit_proj and proj_title:
            new_proj = {
                ...
                "data_freshness_days": (datetime.date.today() - datetime.datetime.strptime(source_date, "%d-%m-%Y").date()).days if source_date else 0,
                "confidence":       conf_inp,
...
def _tab_signals():
    ...
    with st.expander("📝 Log New Signal"):
        ...
        with st.form("add_signal_form"):
            ...
            f3, f4, f5 = st.columns(3)
            sig_date = f3.text_input("Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
...
def _tab_add_contractor():
    ...
    if submit_con and name:
        ...
        new_con = {
            ...
            "type": note, "rating": "Not rated", "last_seen_date": datetime.date.today().strftime("%d-%m-%Y"),
...
def _tab_contacts():
    ...
    with col2:
        ...
        biz_rows = [
            ("Annual Turnover",      f'₹{c.get("annual_turnover_cr",0):,} Cr'),
            ("Order Book",           f'₹{c.get("order_book_cr",0):,} Cr'),
...
def _tab_timeline():
    ...
    if milestones and PLOTLY_OK:
        ...
        for ms in milestones:
            try:
                t_dt = datetime.datetime.strptime(ms["target_date"], "%d-%m-%Y")
    ...
    with st.expander("📝 Milestone Entry Form"):
        with st.form(f"ms_form_{selected_pid}"):
            ...
            ms_target = mf3.text_input("Target Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
...
def _tab_add_edit_data():
    ...
    with t_add_con:
        ...
        if submit_con and name:
            ...
            new_con = {
                ...
                "last_seen_date": datetime.date.today().strftime("%d-%m-%Y"),
    ...
    with t_add_proj:
        ...
        with st.form("add_project_full_form"):
            ...
            start_dt    = q4.text_input("Start Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
            end_dt      = q5.text_input("End Date (DD-MM-YYYY)",   (datetime.date.today()+datetime.timedelta(days=730)).strftime("%d-%m-%Y"))
            ...
            source_date = t2.text_input("Source Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
        ...
        if submit_proj and proj_title and contract_val > 0:
            try:
                freshness = (datetime.date.today() - datetime.datetime.strptime(source_date, "%d-%m-%Y").date()).days
            except Exception:
                freshness = 0
            new_proj = {
                "project_id": _project_id(sel_con, proj_title, state),
    ...
    with t_add_sig:
        ...
        with st.form("add_signal_full"):
            ...
            sig_date = s3.text_input("Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
    ...
    with t_add_risk:
        ...
        with st.form("add_risk_full"):
            ...
            rf_date = r4.text_input("Date (DD-MM-YYYY)", datetime.date.today().strftime("%d-%m-%Y"))
