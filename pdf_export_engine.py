...
def _ts_ist(fmt: str = "%d-%m-%Y %H:%M IST") -> str:
    return _now_ist().strftime(fmt)

def _ts_filename() -> str:
    return _now_ist().strftime("%d-%m-%Y_%H%MIST")
    ...
    def add_alert_table(self, alerts: list[dict]):
        ...
        for a in alerts:
            rows.append([
                ...
                a.get("category", "—"),
                a.get("title", "—")[:60],
                a.get("time", "—").strftime("%d-%m-%Y %H:%M") if hasattr(a.get("time"), "strftime") else str(a.get("time", "—"))[:16],
