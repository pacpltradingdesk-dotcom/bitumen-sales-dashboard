    ...
    def check_data_quality() -> List[dict]:
        ...
        for key in expected_keys:
            ...
            if val is None:
                ...
                price_issues.append(
                    f"Out of range: {key}=₹{val:,} "
                    f"(expected ₹{ps['min_bitumen_inr_per_mt']:,}–₹{ps['max_bitumen_inr_per_mt']:,})"
                )
...
class DataValidator:
    ...
    def validate_price(value: Any, label: str = "price",
        ...
        if value is None:
            ...
            issues.append(f"{label}=₹{value:,.0f} below min ₹{min_inr:,.0f}")
        elif value > max_inr:
            issues.append(f"{label}=₹{value:,.0f} above max ₹{max_inr:,.0f}")
